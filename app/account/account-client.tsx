...
/**
 * A plain-English sentence telling the customer where their request stands.
 *
 * The status pill alone is jargon ("Pending review"); this is the line that
 * makes clear a chosen date is still only a preference until confirmed.
 */
/**
 * True when the record chain was generated by a direct invoice rather than a
 * real quote-then-accept flow.
 *
 * `createDirectInvoice` fabricates a quotation marked sent + accepted so the
 * internal chain stays consistent. The customer never saw or accepted that
 * document, so the portal must not present it as a quotation they agreed to.
 */
/**
 * Explains what each status colour means, so a customer can read a card's
 * state at a glance. Mirrors the shared status tones: blue = received,
 * amber = waiting/not confirmed, green = confirmed, grey = completed.
 */
function StatusColourGuide() {
  const entries: { dot: string; title: string; detail: string }[] = [
    {
      dot: "bg-sky-500",
      title: "Request received",
      detail: "We have your request.",
    },
    {
      dot: "bg-amber-500",
      title: "Waiting",
      detail: "The business needs to confirm.",
    },
    {
      dot: "bg-emerald-500",
      title: "Confirmed",
      detail: "Your appointment or job is booked.",
    },
    {
      dot: "bg-stone-400",
      title: "Completed",
      detail: "The work is finished.",
    },
  ];
  ...
  return (
    <ul className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
      {entries.map((entry) => (
        ...
        >
          <p className="flex items-center gap-1.5 font-body text-[12px] font-bold text-on-surface">
            <span
              aria-hidden
              className={`h-2.5 w-2.5 shrink-0 rounded-full ${entry.dot}`}
            />
            {entry.title}
          </p>
...
function isDirectInvoiceRecord(booking: CustomerBooking): boolean {
  return booking.createdSource === "invoice_direct";
}

function customerStatusSentence(booking: CustomerBooking): string | null {
  // Once there is a job or a quote to act on, the pill and the detail rows
  // already tell the story.
  if (customerJobDisplayStatus(booking)) return null;

  if (
    booking.quotation?.status === "sent" &&
    !booking.quotation.customerDecision
  ) {
    return "Your quote is ready. Review it and let us know your decision.";
  }

  switch (booking.status) {
    case "pending":
      return CUSTOMER_COPY.awaitingConfirmation;
    case "owner_proposed":
      return "A new inspection time has been proposed. Please accept or suggest another.";
    case "scheduled":
      return CUSTOMER_COPY.inspectionConfirmed;
    case "completed":
      return "Your inspection is complete.";
    case "cancelled":
      return "This inspection request was cancelled.";
    default:
      return null;
  }
}
    ...
    <>
      {tab !== "profile" ? (
        <div>
          ...
          {tab === "jobs" ? (
            <p className="mt-1.5 max-w-[60ch] font-body text-[13px] leading-relaxed text-on-surface-variant">
              Track your site inspection requests, quotes, confirmed work, and
              completed jobs.
            </p>
          ) : null}
          {tab === "requests" || tab === "jobs" ? <StatusColourGuide /> : null}
        </div>
      ) : null}
      ...
      <div className={tab === "profile" ? "" : "mt-5"}>
        ...
        {tab === "requests" ? (
          <BookingsList
            slug={slug}
            scope="active"
            emptyHint="You have no active site inspections."
        ...
        {tab === "jobs" ? (
          <BookingsList
            slug={slug}
            scope="history"
            emptyHint="No past requests or jobs yet."
            focusRequestId={focusRequestId}
          />
        ) : null}
        {tab === "notifications" ? (
      ...
      <div className="rounded-2xl border border-stone-200 bg-white p-4 shadow-sm sm:rounded-3xl sm:p-6">
        ...
        <div className="mt-5 flex flex-col gap-3 border-t border-stone-100 pt-5 sm:flex-row sm:items-center sm:justify-between">
          <p className="font-body text-[12px] text-on-surface-variant">
            Changes apply to your next inspection request.
          </p>
          <button
            type="submit"
            disabled={saving}
...
function BookingCard({
  ...
  const { getIdToken, activeBookingSlug } = useCustomerAuth();
  ...
  const statusSentence = customerStatusSentence(booking);
  ...
  return (
    ...
    >
      <div
        className={`border-l-4 bg-gradient-to-r via-white to-white p-4 sm:p-5 ${
          booking.status === "cancelled"
            ? "border-l-stone-400 from-stone-100"
            : booking.status === "completed"
              ? "border-l-stone-400 from-stone-100"
              : booking.status === "scheduled"
                ? "border-l-emerald-500 from-emerald-50"
                : booking.status === "pending" ||
                    booking.status === "owner_proposed" ||
                    booking.status === "awaiting_decision"
                  ? "border-l-amber-500 from-amber-50"
                  : "border-l-primary from-primary/[0.06]"
        }`}
      >
        <div className="flex items-start gap-3">
          <span className="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-primary/10 text-primary ring-1 ring-primary/15">
        ...
        {statusSentence ? (
          <p
            className={`mt-3 flex items-start gap-1.5 rounded-xl border px-3 py-2 font-body text-[12px] font-semibold ${
              booking.status === "scheduled"
        ...
        !expanded &&
        !awaitingQuotationDecision &&
        !isDirectInvoiceRecord(booking) ? (
      ...
      {expanded ? (
        <div className="space-y-3 border-t border-stone-200/80 bg-[#faf8f5] px-4 py-4 sm:px-5">
          ...
          {/*
            A direct invoice fabricates an auto-accepted quotation for the
            internal record chain. The customer never saw it, so only show a
            quotation the customer was actually sent.
          */}
          {booking.quotation?.pdfUrl && !isDirectInvoiceRecord(booking) ? (
          ...
          {/*
            What the customer originally submitted. Collapsed by default so the
            current status leads, and the old request details do not compete
            with it.
          */}
          <details className="group rounded-xl border border-stone-200 bg-white/60">
            <summary className="flex cursor-pointer list-none items-center gap-2 px-3 py-2.5 font-body text-[12px] font-bold text-on-surface-variant transition-colors hover:text-on-surface">
              <span className="material-symbols-outlined text-[18px] transition-transform group-open:rotate-90">
                chevron_right
              </span>
              {CUSTOMER_COPY.originalRequestDetails}
            </summary>
          ...
          {formatBudgetAud(booking.budgetAud) ? (
            <BookingDetailRow
              icon="payments"
              label={CUSTOMER_COPY.budgetProvided}
                ...
                {hasInspectionDates ? (
                  <ScheduleCategoryPanel category="inspection" audience="customer">
                    {booking.preferredSlots.length > 0 ? (
                      <ScheduleSubsection
                        category="inspection"
                        label="Your preferred inspection times"
                        hint="The times you asked for — these were preferences, not confirmed."
                      >
                        <ScheduleSlotsList
                    ...
                    {booking.ownerProposedSlots.length > 0 ? (
                      <ScheduleSubsection
                        ...
                        label={
                          booking.status === "owner_proposed"
                            ? "Business proposed inspection times — pick one"
                            : "Business proposed inspection times"
                        }
...
/**
 * Where a notification came from, and whether it needs the customer to act.
 * Makes the source explicit instead of presenting every update identically.
 */
function notificationSource(
  note: NotificationRecord,
): { label: string; className: string } {
  // Anything the customer must respond to leads with the call to action.
  if (
    note.type === "request_proposed" ||
    note.type === "quotation_sent" ||
...
function NotificationsSection({ slug }: { slug: string }) {
  ...
  // Only block the whole page when there is nothing to show. If notifications
  // are already loaded, a failed refresh is reported inline below instead of
  // hiding them.
  ...
  return (
    <div className="space-y-3">
      <DeleteConfirmModal
        ...
        description="This will permanently remove all updates from your notification list. You can still view your work from the Inspections and History tabs."
      ...
      {/* Refresh failed, but we still have notifications worth showing. */}
      {error ? (
        <div className="flex flex-wrap items-center gap-2 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2">
          <p className="font-body text-[12px] font-semibold text-amber-900">
            Could not refresh notifications. Showing the latest we have.
          </p>
          <button
            type="button"
            onClick={() => void reload()}
            className="ml-auto inline-flex items-center gap-1 rounded-lg border border-amber-300 bg-white px-2.5 py-1 font-body text-[12px] font-bold text-amber-900"
          >
            <span className="material-symbols-outlined text-[14px]">
              refresh
            </span>
        ...
        {notifications.map((note) => (
          ...
          >
            ...
            >
              ...
              <span className="min-w-0 flex-1">
                <span
                  className={`inline-flex items-center rounded-full border px-2 py-0.5 font-body text-[10px] font-bold uppercase tracking-wider ${notificationSource(note).className}`}
                >
                  {notificationSource(note).label}
                </span>
                <span className="mt-1 block font-body text-[13px] font-bold text-on-surface">
                  {note.title}
                </span>
