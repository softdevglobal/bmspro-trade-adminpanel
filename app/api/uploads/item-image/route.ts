...
import { uploadItemImage } from "@/lib/onboarding/services/upload";
  ...
  try {
    ...
    if (!businessId || (role !== "owner" && role !== "admin")) {
      return {
        ok: false as const,
        status: 403,
        error: "You do not have permission to upload item images.",
      };
    }
...
export async function POST(request: Request) {
  try {
    ...
    const result = await uploadItemImage(buffer, file.type || "", {
    ...
    console.error("POST /api/uploads/item-image failed:", error);
    return NextResponse.json(
