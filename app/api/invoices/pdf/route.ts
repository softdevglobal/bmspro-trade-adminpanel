...
import { getBusinessInvoicePdf } from "@/lib/invoices/server";
import { NextResponse } from "next/server";

export const runtime = "nodejs";

async function requireInvoiceReader(request: Request) {
...
export async function GET(request: Request) {
  const auth = await requireInvoiceReader(request);
  if (!auth.ok) {
  ...
  const result = await getBusinessInvoicePdf(auth.businessId, quotationId);
  if (!result.ok) {
    return NextResponse.json(
