...
A **Next.js 16 (App Router)** admin portal for trade businesses. It covers super-admin tenant management, business-owner dashboards (requests, team, services, customers), a public customer booking engine, a **call-center REST API** (`/api/callcenter/*`) for platform-wide agents, custom admin password reset (6-digit code), and transactional email via ZeptoMail.

### Feature quick reference

| Feature | UI | API / server | Docs section |
|---|---|---|---|
| Owner **Add Inspection** (4-step modal) | `components/add-inspection-modal.tsx` | `POST /api/requests` | [§8](#get--post-apiinspection-requests), [§11](#12-dashboard-pages) |
...
│   │   ├── requests/    # Inspection request board
...
| `business_owner` | Their own services, team, requests, customers |
...
| `requests/{id}` | Auto UUID | Service requests |
...
| Owner **Add Inspection** modal | `ensureCustomerAccount()` in `POST /api/requests` | Default **`00001111`** if new Firebase Auth user | Welcome email with credentials when email is **new to `customers` collection** |
...
services/{serviceId}
  └──► requests/{id}        (serviceId field)

customers/{uid}
  └──► requests/{id}        (customerId field)
...
// Update a field
await adminDb.collection("requests").doc(id).update({ status: "scheduled" });
...
| `requests/{id}` | Super admin, owning business, or owning customer | Never |
...
| **Business owner** | Owners + admins | `businessId` JWT claim + `role` ∈ `"owner"` or `"admin"` — includes `GET`/`POST /api/requests`, team, services, notifications |
...
| Method | Does |
|---|---|
| `GET` | `listInspectionRequests(businessId)` → reads up to 80 requests from `requests` ordered by `createdAt` desc |
...
3. `createInspectionRequest(businessId, input, { customerId })` — writes `requests` → **inspection notification emails** to business + customer (`notifyBusinessOfNewRequest`, `notifyCustomerOfNewRequest`).
...
> **Owner vs public:** Business owners use **`POST /api/requests`** (no `slug`; auto-creates customer with default password `00001111` + welcome email when new). Public customers use this route with `slug` and optional sign-in.
...
| Function | Does |
|---|---|
| `mapBookingDoc(doc)` | Maps `requests` doc → `InspectionRequestDetail` |
| `loadBusinessSummaries(ids)` | Batch reads `businesses` docs → name + slug map |
| `GET` | Queries `requests` by `customerId` AND by `customer.email` → merges, deduplicates → attaches business names |

---

### `PATCH /api/customer/jobs/[id]`

**Auth:** Customer Bearer token  
**File:** `app/api/customer/jobs/[id]/route.ts`
...
| `createInspectionRequest(businessId, input, opts)` | business ID, validated input, `{ customerId? }` | Creates `pending` request → notifies business + customer | write `requests`; read `businesses`, `services` |
| `listInspectionRequests(businessId)` | string | Reads up to 80 requests ordered `createdAt` desc | read `requests` |
| `getInspectionRequest(id, businessId)` | ids | Reads one request; verifies business ownership | read `requests` |
| `applyOwnerAction(id, businessId, action)` | `OwnerAction` | Status transitions + side-effects (see table below) | update `requests`; side-effects via notifications |
| `customerAcceptProposedSlot(id, identity, slot)` | customer identity + slot | `owner_proposed` → `scheduled`; notifies business | update `requests` |
...
Client (business owner)
  ...
  └── POST /api/requests
        ...
        │     ├── adminDb.collection("requests").add() ──► Firestore WRITE
...
Customer
  ...
  │
  ├── Query 1: adminDb.collection("requests")
  │     .where("customerId","==",uid).get()    ──► Firestore READ
  │
  ├── Query 2: adminDb.collection("requests")
...
| Component | Description |
|---|---|
| `requests-board.tsx` | Inspection request board: filters, detail drawer, owner actions, **Add Inspection** + Refresh buttons |
| `add-inspection-modal.tsx` | 4-step owner modal (job → address → dates → contact); submits to `POST /api/requests` |
...
| `/dashboard/requests` | Owner | Inspection request board; **Add Inspection** opens 4-step modal; auto-creates customer + emails |
| `/dashboard/jobs` | Owner | Job bookings board |
...
Next.js App Router
  ...
  └── components/
        ...
        requests-board.tsx → board + Add Inspection button
...
Firebase (Backend)
  ...
  ├── Firestore                  businesses, users, customers, requests, …
